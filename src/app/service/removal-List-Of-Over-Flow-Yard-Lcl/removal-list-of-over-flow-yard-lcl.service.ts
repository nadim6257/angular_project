...
export class RemovalListOfOverFlowYardLclService {
  ctgPortTitle="CHITTAGONG PORT AUTHORITY,CHITTAGONG"
  title:any ="       Removal Tally of Overflow Yard (LCL)";
  ctmsTitle:any ="       Removal Tally of Overflow Yard (LCL)";
  ...
  getRemovalListOfOverFlowYardLcl(assignDate:any){
    return this.httpClient.get('http://192.168.16.243:8093/importReports/RemovalListOfOverFlowYardLcl/' + assignDate+"/");
  }

  getResultWithExcel(resultList:any,modify:any){
    let reportTitle="";
    if(modify=="all"){
      reportTitle=this.ctmsTitle;

    }
    else if(modify=="overflow"){
      reportTitle=this.title;
    }
   
    let workbook = new Workbook();
    let worksheet = workbook.addWorksheet("Removal Tally of Overflow Yard (LCL)");
   ...
   workbook.xlsx.writeBuffer().then((data:any) => {
    ...
    fs.saveAs(blob, 'RemovalTallyOfOverflowYardReportLcl.xlsx');
  });
