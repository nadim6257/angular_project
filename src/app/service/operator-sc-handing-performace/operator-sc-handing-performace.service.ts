...
export class OperatorScHandingPerformaceService {
  fileName= 'ExcelSheet.xlsx';
  ctgPortTitle="     CHITTAGONG PORT AUTHORITY,CHITTAGONG"
  title:any ="OPERATOR'S SC HANDLING PERFORMANCE REPORT";
  header:any = ["SlNo","Operator", "Import Receiving", "Keep Down / Delivery","Delivery (OCD / Off Dock)","Shifting","Total Handing Boxes"];

  constructor(
    private httpClient: HttpClient
  ) { }

  getOperationScPerformanceHandlingList(shift:any,fromDate:any,fromTime:any,toDate:any,toTime:any){
    ...
    return this.httpClient.get(`http://192.168.16.243:8093/importReports/OperatorScHandlingPerformance/` +shift+"/"+fromDate+"/"+fromTime+"/"+toDate+"/"+toTime+"/"); 
  }

  getResultWithExcel(resultList:any,shift:any,fromDate:any,toDate:any){
    // Create workbook and worksheet
    let workbook = new Workbook();
    let worksheet = workbook.addWorksheet("OPERATOR'S SC HANDLING PERFORMANCE REPORT");
    ...
    // Add Data and Conditional Formatting
     ...
     for(let result of resultList){
      i++;
        ...
        let row = worksheet.addRow([i,result["placed_by"],result["impRcv"],result["keepDlv"],result["dlvOcdOffDock"],result["shift"],result["totalHandingBox"]]);
        let color = 'FF99FF99';
   ...
   workbook.xlsx.writeBuffer().then((data:any) => {
    ...
    fs.saveAs(blob, 'OperatorScHandlingPerformaceReport.xlsx');
  });
